...
process_fastq_file <- function(fastq_file) {
  ...
  trimmed_fastq <- trim_sequences(fastq_data)
  ...
  # Escriure l'arxiu fastq processat
  output_file <- paste0(tools::file_path_sans_ext(fastq_file), "_processed.fastq")
  writeFastq(normalized_fastq, file = output_file, compress = FALSE)
}
...
lapply(fastq_paths, process_fastq_file)
